...
lt_vec3i_t lt_v3iabs(lt_vec3i_t v) {
	return LT_VEC3I(lt_abs(v.x), lt_abs(v.y), lt_abs(v.z));
}
...
lt_vec3i_t lt_v3imin(lt_vec3i_t a, lt_vec3i_t b) {
	return LT_VEC3I(lt_min(a.x, b.x), lt_min(a.y, b.y), lt_min(a.z, b.z));
}

static LT_INLINE
lt_vec3i_t lt_v3imax(lt_vec3i_t a, lt_vec3i_t b) {
	return LT_VEC3I(lt_max(a.x, b.x), lt_max(a.y, b.y), lt_max(a.z, b.z));
}

static LT_INLINE
lt_vec3i_t lt_v3iclamp(lt_vec3i_t v, lt_vec3i_t min, lt_vec3i_t max) {
	return LT_VEC3I(lt_clamp(v.x, min.x, max.x), lt_clamp(v.y, min.y, max.y), lt_clamp(v.z, min.z, max.z));
}

static LT_INLINE
lt_vec3i_t lt_v3iclampi(lt_vec3i_t v, i32 min, i32 max) {
	return LT_VEC3I(lt_clamp(v.x, min, max), lt_clamp(v.y, min, max), lt_clamp(v.z, min, max));
}
